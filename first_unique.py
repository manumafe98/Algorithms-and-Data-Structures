...
def solution(A):
    ...
    for idx, val in enumerate(A):
        if val in unique_dict:
            # remove duplicateds
            unique_dict.pop(val)
            # if the value appears three times if would count as a first time appearances
            # so adding it to another dict we prevent that
            poped_dict[val] = True
        elif val in poped_dict:
            pass
        else:
            unique_dict[val] = idx

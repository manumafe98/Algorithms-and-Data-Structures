...
def solution(A, B):
    # Array of fishes going downstream
    downstream_fist = []
    # The count of the eaten fish
    count = 0

    for idx, val in enumerate(A):
        if B[idx] == 1:
            downstream_fist.append(val)
        else:
            if len(downstream_fist) == 0:
                pass
            else:
                # Looping over the fishes in the downstream array
                while len(downstream_fist) != 0:
                    if val > downstream_fist[-1]:
                        downstream_fist.pop()
                        count += 1
                    else:
                        # The last fish in downstream eats the val so we break the while loop 
                        # to continue with the for loop
                        count += 1
                        break
    return len(A) - count
